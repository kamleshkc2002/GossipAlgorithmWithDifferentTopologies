...
case class startGossiping(actor:ActorRef, algorithm:String, numNodes:Int, dcnode:Int)
...
case class Gossip(id:Int, starttime:Long, dcnode:Int)
case class Pushsum(sum:Double, weight:Double, starttim:Long, dcnode:Int)
...
object project2bonus {
	def main (args: Array[String]): Unit = {
		if (args.length != 4) {
			...
			master ! startGossiping(actorslist(0), algorithm, numNodes, dcnode)
		}
...
case class ActorClass () extends Actor {
	...
	case Gossip(callerid:Int, starttime:Long, dcnode:Int) => {
		if (rumorcounter < rumorthreshhold && myID != dcnode) {
			...
			actorslist(neighbors(randomPlayer)) ! Gossip(myID, starttime, dcnode)
			self ! Gossip(myID, starttime, dcnode)
		}
		else {
	...
	case Pushsum(news:Double, neww:Double, starttime:Long, dcnode:Int) => {
		if (myID != dcnode) {
...
//		println("newratio="+newratio+"s="+s+"w="+w)
		if ((rumorcounter == 1) || (Math.abs((oldratio-newratio)) > math.pow(10, -10))) {
			...
			actorslist(neighbors(randomPlayer)) ! Pushsum(s, w, starttime, dcnode)
...
class MasterClass extends Actor{
	...
	def receive = {
		...
		case CheckForTermination(id:Int, count:Int, starttime:Long) => {
			...
			if(visitednodes.length == (numNodes-1)) {
				println("\nNodes visited = ")
				var i:Int = 0
				while (i < visitednodes.length) {
